#!/usr/bin/env python3
"""Comprehensive A/B test: YOLO-Master-v0.1-N (baseline) vs -P2 at imgsz=640.
...
Usage:
    python scripts/compare_p2_vs_baseline.py                 # full A/B (val + latency)
    python scripts/compare_p2_vs_baseline.py --no-latency    # accuracy only (safe on busy GPU)
    python scripts/compare_p2_vs_baseline.py --no-val        # latency only (needs idle GPU)
    python scripts/compare_p2_vs_baseline.py --device 1      # pick an idle GPU
"""
...
MODELS = {
    ...
    "v0.1-N-P2":        ROOT / "v0.1-N-TPH-640/VisDrone_v0.1-N-TPH/weights/best.pt",
}
...
def fmt_table(results, args):
    a, b = results  # baseline, p2
    L = []
    L.append("\n" + "#" * 78)
    L.append(f"# YOLO-Master  v0.1-N  vs  v0.1-N-P2   (VisDrone val, imgsz={args.imgsz})")
    L.append("#" * 78)
    ...
    # per-class: where does P2 help?
    if "per_class_map" in a:
        L.append("\nPer-class mAP50-95 (sorted by P2 gain — small classes should top):")
        L.append(f"{'class':<18}{'baseline':>12}{'P2':>12}{'delta':>10}")
        L.append("-" * 52)
...
def main():
    ...
    p.add_argument("--out", default=str(ROOT / "scripts/reproduce/results/p2_vs_baseline_640.md"))
    args = p.parse_args()
